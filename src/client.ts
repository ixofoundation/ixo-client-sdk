...
export function getTemplate(tplRecOrDid:string): string {
    const
        tplDoc =
...
export function makeRpcMsg(method:string, templateName:string, data:any, signature:string): any {
	return {
        jsonrpc: '2.0',
        method,
        id: generateTxId(),
...
export function dashifyUrl(urlStr:string): string {
    urlStr.replace(/^(https?:\/\/)([^/]+)(\/.*)?/,(_, proto, host, path) => proto + host.replace('_', '-') + (path || '');
	return urlStr;
}
