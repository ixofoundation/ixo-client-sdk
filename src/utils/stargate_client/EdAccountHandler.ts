...
import { decodePubkey } from './customPubkey';
...
export function accountFromAny(input: Any): Account {
  ...
  switch (typeUrl) {
    ...
    case '/cosmos.vesting.v1beta1.ContinuousVestingAccount': {
      const baseAccount = ContinuousVestingAccount.decode(value)
        ?.baseVestingAccount?.baseAccount;
      assert(baseAccount);
      return accountFromBaseAccount(baseAccount);
    }
    case '/cosmos.vesting.v1beta1.DelayedVestingAccount': {
      const baseAccount = DelayedVestingAccount.decode(value)
        ?.baseVestingAccount?.baseAccount;
      assert(baseAccount);
      return accountFromBaseAccount(baseAccount);
    }
    case '/cosmos.vesting.v1beta1.PeriodicVestingAccount': {
      const baseAccount = PeriodicVestingAccount.decode(value)
        ?.baseVestingAccount?.baseAccount;
      assert(baseAccount);
      return accountFromBaseAccount(baseAccount);
    }
